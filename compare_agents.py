...
def load_player(script_path):
    ...
    return path.stem, module.setup_ai()
...
def run_match(player_a_path, player_b_path, max_round, initial_stack, small_blind):
    ...
    name_a, player_a = load_player(player_a_path)
    name_b, player_b = load_player(player_b_path)
    config.register_player(name=name_a, algorithm=player_a)
    config.register_player(name=name_b, algorithm=player_b)
...
def compare(player_a_path, player_b_path, games, max_round, initial_stack, small_blind):
    summary = {}
    for script_path in (player_a_path, player_b_path):
        name = Path(script_path).stem
        summary[name] = {"wins": 0, "ties": 0, "total_stack": 0}
...
def print_summary(summary, games, max_round, initial_stack, small_blind):
    ...
    print("-" * 70)
    for name, stats in sorted(summary.items(), key=lambda item: (-item[1]["wins"], -item[1]["total_stack"])):
        avg_stack = float(stats["total_stack"]) / games
        print("%-35s %8d %8d %14.2f" % (name, stats["wins"], stats["ties"], avg_stack))
