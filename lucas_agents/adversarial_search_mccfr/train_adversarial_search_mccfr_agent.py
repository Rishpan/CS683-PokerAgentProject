...
DEFAULT_POLICY_PATH = os.path.join(
    PROJECT_ROOT,
    "lucas_agents",
    "adversarial_search_mccfr",
    "adversarial_search_mccfr_policy.json",
)


def parse_args():
  parser = argparse.ArgumentParser(description="Train the adversarial-search MCCFR agent.")
...
def build_opponent(game_index, policy_path):
  if game_index % 11 == 0:
    return AdversarialSearchMCCFRAgent(policy_path=policy_path, training_enabled=False, exploration=0.0)
  if game_index % 7 == 0:
...
def main():
  args = parse_args()
  learner = AdversarialSearchMCCFRAgent(policy_path=args.policy_path, training_enabled=True)
  total_rounds = 0
  ...
  for game_index in range(1, args.games + 1):
    ...
    config.register_player(name="adversarial_search_mccfr", algorithm=learner)
    ...
    print(
        ...
        f"stack={next(player['stack'] for player in result['players'] if player['name'] == 'adversarial_search_mccfr')}"
    )
