...
def parse_args():
  parser = argparse.ArgumentParser(description="Train the conservative adversarial-search MCCFR v2 agent.")
...
def build_opponent(game_index, policy_path):
  if game_index % 13 == 0:
    return AdversarialSearchMCCFRV2Agent(policy_path=policy_path, training_enabled=False, exploration=0.0)
  if game_index % 11 == 0:
...
def main():
  args = parse_args()
  learner = AdversarialSearchMCCFRV2Agent(policy_path=args.policy_path, training_enabled=True)
  total_rounds = 0
  ...
  for game_index in range(1, args.games + 1):
    ...
    config.register_player(name="adversarial_search_mccfr_v2", algorithm=learner)
    ...
    print(
        ...
        f"stack={next(player['stack'] for player in result['players'] if player['name'] == 'adversarial_search_mccfr_v2')}"
    )
