...
def parse_args():
  parser = argparse.ArgumentParser(description="Train the learnable discounted MCCFR agent.")
...
def build_opponent(game_index, policy_path):
  if game_index % 9 == 0:
    return LearnableDiscountedMCCFRAgent(policy_path=policy_path, training_enabled=False, exploration=0.0)
  if game_index % 5 == 0:
...
def main():
  args = parse_args()
  learner = LearnableDiscountedMCCFRAgent(policy_path=args.policy_path, training_enabled=True)
  total_rounds = 0
  ...
  for game_index in range(1, args.games + 1):
    ...
    config.register_player(name="learnable_discounted_mccfr", algorithm=learner)
    ...
    print(
        ...
        f"stack={next(player['stack'] for player in result['players'] if player['name'] == 'learnable_discounted_mccfr')}"
    )
